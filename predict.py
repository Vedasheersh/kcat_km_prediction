...
def write_outputs_to_file(outputs, output_file, prediction_type):
    ...
    if prediction_type=='KCAT': f.write('tag,kcat(s^-1)\n')
    elif prediction_type=='KM': f.write('tag,KM(M)\n')
    
    for tag, pred in outputs.items():
        f.write(f'{tag},{round(pred,3)}\n ')
    f.close()
...
def main(args):
    ...
    for tag, (substrate_feat, sequence_feat) in tqdm(features.items()):
        ...
        outputs[tag] = float(prediction.numpy().flatten()[0])**10
